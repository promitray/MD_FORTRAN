program main
      ...
      allocate (a(n,n),rx(n),ry(n),rz(n),vx(n),vy(n),vz(n))
      ...
      call  verlet_algorithm(rx,ry,rz) 
end program main
    
    subroutine verlet_algorithm(rx,ry,rz) 
      implicit none 
      integer::i,n=108,k,itime = 70000
      real(8)::v,pot_lj,KE,T,m,delta =0.005d0,T_req = 85.0d0
      real(8)::rx(200),ry(200),rz(200),fx(200),fy(200),fz(200),vx(200),vy(200),vz(200 )
      m=39.948
      !allocate (rx(n),ry(n),rz(n),fx(n),fy(n),fz(n),vx(n),vy(n),vz(n))
     ...
     !!main loop
      do k=1,itime
       ...
       if (k<10000)then
        do i=1,n
         vx(i) = vx(i) * sqrt(T_req/T)
         vy(i) = vy(i) * sqrt(T_req/T)
         vz(i) = vz(i) * sqrt(T_req/T)
        end do
       end if
       call calc_pot(n,rx,ry,rz,v)
       write(57,*)k*delta,v,KE,v+KE
     ...
     do i=1,n-1
      do j=i+1,n
        ...
        if(rijsq<rcutoffsq) then
          pot_lj= pot_lj + sr12 -sr6 +(3.83738839608178386E-003)           
        endif
     end do
   end do 
